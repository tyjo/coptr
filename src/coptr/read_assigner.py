...
class ReadAssigner(object):
    ...
    def __init__(self, X, prior_counts):
        ...
        assert np.all(self.X.sum(axis=1) > 0), "each read must have at least one valid mapping"
    ...
    def update_phi(self):
        ...
        phi = csr_matrix((np.exp(log_phi.data), (log_phi.row, log_phi.col)), shape=log_phi.shape)
    ...
    def run_vi(self, tol=1E-3, verbose=False):
        ...
        while np.abs(prv_elbo - elbo) > tol:
            ...
            assert elbo >= prv_elbo - 1E-3, "elbo must be strictly increasing ({} > {})".format(elbo, prv_elbo)
...
if __name__ == "__main__":
    ...
    for i in range(100):
        ...
        prior = Z[multimapped_reads:,].sum(axis=0)
        ...
        print("\t{} of reads correctly mapped".format(correct_assignments / len(assignments)))
    print("done!")
